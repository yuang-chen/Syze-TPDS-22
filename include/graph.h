...
template <class type> struct graph {
  intE *csc_offset{nullptr};
  intV *csc_index{nullptr};
  ...
  intE **updateBinPointers; // running address (counters) pointing to next write
                            // location
  intE **destIdBinPointers;
  unsigned int ***indWeightBins; // weight bins
  intV ***indDestIdBins;         // dest ID bins (dense, fixed order)
  intV **
      *sparseDestIdBins; // sparse dest ID bins (empty, to be filled on scatter)
  ...
  std::atomic<intV>
      partListPtr;   // # partitions to be processed (scatter or gather)
  intV **activeBins; // array of bins with messages

  std::atomic<intV> denseSub;

  std::vector<unsigned> sub_map;
  std::vector<unsigned> sub_offset;

  unsigned inline locateSub(unsigned vertex, unsigned offset) {
    auto const orig_sub = vertex >> offset;
    auto const curr_off = sub_offset[orig_sub];
    auto const div_sub =
        ((1 << (offset - curr_off)) - 1) & (vertex >> curr_off);
    return sub_map[orig_sub] + div_sub;
    // return vertex >> offset;
  }
};

template <class graph> int read_mix(char *filename, graph *G) {
  ...
  G->csc_offset = new intE[G->numVertex + 1];
  fread(G->csc_offset, sizeof(intE), G->numVertex, graphFile);
  G->csc_offset[G->numVertex] = G->numEdges;

  G->csc_index = new intE[G->numEdges];
  fread(G->csc_index, sizeof(intE), G->numEdges, graphFile);
...
template <class graph> int read_csr(char *filename, graph *G) {
  FILE *graphFile = fopen(filename, "rb");
  if (graphFile == NULL) {
    fputs("file error\n", stderr);
    return -1;
...
template <class graph> void write_csr(char *filename, graph *G) {
...
template <class graph> void printGraph(graph *G) {
...
template <class graph> void transposeCSR(graph *G1) {
...
template <class graph> void findOutDeg(graph *G) {
...
#pragma omp atomic
  ...
  for (intV i = 0; i < G->numVertex; i++) {
    ...
    if (G->outDeg[i] > G->numEdges / G->numVertex) {
      vcount++;
      ecount += G->outDeg[i];
    }
  }
  std::cout << (float)vcount / G->numVertex << " "
            << (float)ecount / G->numEdges << '\n';
  return;
}

template <class graph> void initGraph(graph *G) {
...
template <class graph> void freeMem(graph *G) {
  if (G->VI != NULL) {
    delete[] G->VI;
    G->VI = NULL;
  }
...
template <class graph> intV findFrontierSize(graph *G) {
