...
void loadFrontier(graph* G, intV* initFrontier, intV initFrontierSize)
{
    ...
    for (intV i=0; i<initFrontierSize; i++)
    {
        ...
        pId = G->locateSub(vertexId, binOffsetBits); //(vertexId >> binOffsetBits);
    ...
    for (intV i=0; i<initFrontierSize; i++)
    {
        pId = G->locateSub(initFrontier[i], binOffsetBits);//(initFrontier[i] >> binOffsetBits);
        if (G->flag[pId])
            G->flag[pId] = false;
    }
...
void loadFrontierPar(graph* G, intV* initFrontier, intV initFrontierSize)
{
    ...
    for (intV i=0; i<initFrontierSize; i++)
    {
        ...
        G->inFrontier[vertexId] = true;
        intV pId = G->locateSub(vertexId, binOffsetBits); //(vertexId >> binOffsetBits);
    ...
    for (intV i=0; i<initFrontierSize; i++)
    {
        intV pId = G->locateSub(initFrontier[i], binOffsetBits); //(initFrontier[i] >> binOffsetBits);
        if (G->flag[pId])
            G->flag[pId] = false;
    }
    G->frontierSize = initFrontierSize;
...
void scatterVC(graph* G, partitionData* TD, type** updateBins, intV** destIdBins, intE* updateBinPointers, intE* destIdBinPointers, userArg UA)
{
    ...
    for (intV i=0; i<TD->frontierSize; i++)
    {
        ...
        {
            destId = G->EI[j]; 
            destBin = G->locateSub(G->EI[j], binOffsetBits);//(destId >> binOffsetBits);
...
void scatterPC(graph*G, partitionData* TD, type** updateBins, userArg UA)
{
    ...
    for (intV i=0; i<PNG->numVertex; i++)
    {
        ...
        for (intE j=PNG->VI[i]; j<PNG->VI[i+1]; j++){
            userReturn = UA.scatterFunc(PNG->EI[j]); //pageRank[node]
...
#endif
{
    ...
    {
        destId = destIdBin[j];
        updateBinPointer += (destId >> MSB_ROT); //G->locateSub(G->EI[j], binOffsetBits);  //
...
#endif
{
    ...
    {
        destId = destIdBin[j];
        updateBinPointer += (destId >> MSB_ROT); // G->locateSub(destId, binOffsetBits);
        destId = destId & MAX_POS;
#ifdef WEIGHTED
        updateVal = UA.applyWeight(updateBin[updateBinPointer], weightBin[j]);
#else
