...
void initialize(graph* G, int argc, char** argv)
{
    ...
    {  
        if (i + 1 != argc)
        {
            ...
            {                 
                binWidth = ((unsigned int)atoi(argv[i + 1])*1024)/sizeof(float);    // The next value in the array is your value
                count = i;
                i++;    // Move to the next flag
            }
            ...
            {                 
                dynamic = (unsigned int)atoi(argv[i + 1]) == 1;    // The next value in the array is your value
                i++;    // Move to the next flag
            }
        }
    }
    if (argc < 2)
    {
        printf("Usage : %s -d <filename> -v <start vertex>-s <part size> -i <#iterations> -t <threads> -r <rounds> -y <dynamic or not> \n", argv[0]);
        exit(1);
    }
...
void dynamicSplit(graph *G, bool is_dynamic) {
   ...
   // auto ver_deg = G->outDeg;
    ...
    for(unsigned n = 0; n < old_num_sub; n++) {
        ...
        unsigned end = (n + 1) * binWidth > G->numVertex?
                    G->numVertex: (n + 1) * binWidth;
   ...
   //std::cout << "the largest element: " << max_element(sub_div.begin(), sub_div.end()) << '\n';
...
std::tuple<double, double, double, double, double, double> scatter_and_gather_thread(graph* G, userArg UA)  // std::tuple<double, double, double, double, double, double>
{
#ifdef ITERTIME
    float time;
...
//    printf("\n"); 
        ...
        double s_t_max = 0.0;
        double s_t_min = DBL_MAX_10_EXP;
        double s_t_sum = 0.0;
        #pragma omp parallel for schedule(dynamic,1) num_threads(NUM_THREADS) reduction(max:s_t_max) reduction(min:s_t_min) reduction(+:s_t_sum)
...
#endif
            times = timer.elapsed();
            s_t_max = fmax(s_t_max, (double) times.wall/(1e6));
            s_t_min = fmin(s_t_min, (double) times.wall/(1e6));
            s_t_sum += times.wall/(1e6);
        }
...
#endif   
        double g_t_max = 0.0;
        double g_t_min = DBL_MAX_10_EXP;
        double g_t_sum = 0.0;
        g_t_sum = 0.0;
        g_t_max = 0.0;
        g_t_min = DBL_MAX_10_EXP;
        #pragma omp parallel for schedule(dynamic,1) num_threads(NUM_THREADS) reduction(max:g_t_max) reduction(min:g_t_min) reduction(+:g_t_sum)
        for (intV ptr=0; ptr<G->partListPtr; ptr++)
        {
            ...
            times = timer.elapsed();
            g_t_max = fmax(g_t_max, (double) times.wall/(1e6));
            g_t_min = fmin(g_t_min, (double) times.wall/(1e6));
            g_t_sum += (double) times.wall/(1e6);
        }
    ...
    return std::make_tuple(s_t_max, s_t_min, s_t_sum/numActiveBins, g_t_max, g_t_min, g_t_sum/G->partListPtr);
